...
- **`pretty-converter kindle` post-conversion integrity check**: every Kindle build now verifies its own output by default — Calibre re-extracts the finished file's text layer, which is scanned for leaked markup (raw `aid=`/`class="chroma-` KF8 fragments that broken renderers spill into visible text) and for missing content (each source chapter's title must appear in the extracted text; failures name the affected chapters). A damaged file fails the build instead of shipping. `--no-verify` skips the check for very large books. New Go API: `kindle.VerifyConverted` (plus `kindle.Audit`/`kindle.Finding`/`kindle.Severity`).
- **`pretty-converter format`**: converts raw, unstructured `.txt` (a single file or a directory) into clean `.mdx` files — `[X.Y.Z]` frontmatter, Markdown headings, lists, and fenced code blocks — plus a scaffolded `go-pretty-converter.yml`, so a pile of raw notes no longer has to be hand-formatted before it can go through `check`/`build`. Detection is heuristic and deterministic (no AI/LLM, no network access), deliberately conservative to avoid misreading ordinary prose as structure, and staged: it writes files for review rather than rendering a PDF directly. Also prints an advisory (never auto-applied) theme suggestion based on lightweight content signals. Flags: `--out`, `--force`, `--title`/`--author`, `--no-verify` (skip the default re-parse-and-analyze summary of the generated output), `--json`. New Go API: the `format` package (`format.Convert`, `format.Write`, `format.Options`, `format.Report`).
...
- **Kindle output now converts cleanly on real devices**: three root causes of garbled Kindle text (spliced mid-word sentences, `class="chroma-w" aid="4GULHE">`-style raw markup appearing in code blocks) are addressed in the conversion itself. (1) The EPUB stylesheet now ships with CSS custom properties (`var(--pdf-*)`) resolved to concrete values, `calc()` space multipliers computed, and `display: flex/grid` declarations removed — Kindle's KF8 renderer supports none of these, and silently dropping the margin/padding/line-height rules they drive is what makes adjacent text runs collide and splice on device. (2) Kindle builds convert code blocks to plain `<pre><code>` text: Chroma's per-token `<span>` soup (one span per whitespace run included) made Calibre emit hundreds of KF8 `aid` anchor fragments per code block, which old Kindle renderers mangle into visible raw markup; PDF and EPUB builds keep syntax highlighting. (3) Every EPUB chapter now passes a well-formed-XML check at build time — malformed chapters used to be silently "fixed" by Calibre during conversion, which is itself a splicing source. EPUB output also gets the CSS resolution (most EPUB readers ignore `var()`), so the fix benefits `epub` builds too.
...
[0.12.1]: https://github.com/sazardev/go-pretty-converter/releases/tag/v0.12.1
